require(pacman)
pacman::p_load(tidyverse, plumber, pryr)
...
function(ACCENUMB, COLLSITE = NA, ORIGCTY = NA, DECLATITUDE = NA, DECLONGITUDE = NA, ELEVATION = NA, INSTCODE, CROPNAME) {
  ...
  df_final <- checking_process_v2(COMPLETE_data = dta)
...
function(dataset) {
  print("API /upload")
  gc(verbose = T) # Run garbage collection
  # print(paste0("API environment: ", paste0(ls(), collapse = ", ")))
  ...
  df_final <- checking_process_v2(COMPLETE_data = dataset)
  ...
  # print(paste0("API end environment: ", paste0(ls(), collapse = ", ")))
  gc(verbose = T) # Run garbage collection
  return(df_final)
}
